class TrainingTracker {
    ...
    setupEventListeners() {
        ...
        // 過去日付の記録追加
        document.getElementById('pastRecordType').addEventListener('change', () => {
            this.updatePastCustomValueInputs();
        });
        
        document.getElementById('addPastCustomRecord').addEventListener('click', () => {
            this.addPastCustomRecord();
        });
    ...
    showHistoryForDate() {
        ...
        // 過去日付の記録入力セクションを表示
        this.showPastRecordSection(date);
    }
    ...
    // 過去日付の記録入力セクション表示
    showPastRecordSection(date) {
        const section = document.getElementById('pastRecordSection');
        if (!section) return;
        
        section.style.display = 'block';
        this.renderPastPredefinedList(date);
    }
    
    // 過去日付の定期メニューリスト表示
    renderPastPredefinedList(date) {
        const container = document.getElementById('pastPredefinedList');
        if (!container) return;
        
        const records = this.data.records[date] || { predefined: {}, custom: [] };
        ...
        container.innerHTML = this.data.menus.map(menu => {
            const record = records.predefined[menu.id];
            let inputElement = '';
            ...
            switch (menu.type) {
                case 'checkbox':
                    const checked = record?.checked ? 'checked' : '';
                    inputElement = `
                        <input type="checkbox" id="past-menu-${menu.id}" ${checked}
                               onchange="app.updatePastPredefinedRecord('${menu.id}', '${date}', undefined, undefined, this.checked)">
                        <label for="past-menu-${menu.id}">完了</label>
                    `;
                    break;
                case 'single':
                    const value = record?.value || '';
                    ...
                    inputElement = `
                        <input type="number" value="${value}" placeholder="値を入力"
                               onchange="app.updatePastPredefinedRecord('${menu.id}', '${date}', 'value', this.value)">
                        <span class="unit">${unit}</span>
                    `;
                    break;
                case 'distance_time':
                ...
                case 'custom_multi':
                    ...
                    menu.fields?.forEach(field => {
                        ...
                        inputElement += `
                            <div class="field-input">
                                <input type="number" value="${fieldValue}" placeholder="${field.label}"
                                       onchange="app.updatePastPredefinedRecord('${menu.id}', '${date}', '${field.name}', this.value)">
                                <span class="unit">${field.unit}</span>
                            </div>
                        `;
                    });
    ...
    // 過去日付の定期メニュー更新
    updatePastPredefinedRecord(menuId, date, fieldName, value, isChecked) {
        console.log(`UpdatePastPredefinedRecord called:`, { menuId, date, fieldName, value, isChecked });
        
        if (!this.data.records[date]) {
            this.data.records[date] = { predefined: {}, custom: [] };
        }

        if (!this.data.records[date].predefined[menuId]) {
            this.data.records[date].predefined[menuId] = {};
        }

        if (isChecked !== undefined) {
            if (isChecked) {
                this.data.records[date].predefined[menuId] = { checked: isChecked };
                console.log(`Set checked for menu ${menuId} on ${date}:`, isChecked);
            } else {
                console.log(`Removing menu ${menuId} on ${date} (unchecked)`);
                delete this.data.records[date].predefined[menuId];
            }
        } else if (fieldName && value !== undefined && value !== '') {
            this.data.records[date].predefined[menuId][fieldName] = parseFloat(value);
            console.log(`Set field ${fieldName} for menu ${menuId} on ${date}:`, parseFloat(value));
        } else if (fieldName && (value === undefined || value === '')) {
            console.log(`Deleting field ${fieldName} for menu ${menuId} on ${date} (empty value)`);
            delete this.data.records[date].predefined[menuId][fieldName];
            if (Object.keys(this.data.records[date].predefined[menuId]).length === 0) {
                console.log(`Deleting entire menu ${menuId} on ${date} (no fields left)`);
                delete this.data.records[date].predefined[menuId];
            }
        }

        console.log(`Menu ${menuId} after update on ${date}:`, this.data.records[date].predefined[menuId]);
        this.saveData();
        this.renderCalendarView();
        this.showHistoryForDate(); // 履歴表示を更新
    }
    
    // 過去日付のカスタム記録入力フィールド更新
    updatePastCustomValueInputs() {
        const type = document.getElementById('pastRecordType').value;
        const container = document.getElementById('pastCustomValueInputs');
        
        switch (type) {
            case 'single':
                container.innerHTML = `
                    <div class="single-input">
                        <input type="number" id="pastRecordValue" placeholder="値">
                        <input type="text" id="pastRecordUnit" placeholder="単位">
                    </div>
                `;
                break;
            case 'distance_time':
                container.innerHTML = `
                    <div class="multi-input">
                        <input type="number" id="pastDistanceValue" placeholder="距離" step="0.1">
                        <span>km</span>
                        <input type="number" id="pastTimeValue" placeholder="時間" step="0.1">
                        <span>分</span>
                    </div>
                `;
                break;
            case 'weight_reps':
                container.innerHTML = `
                    <div class="multi-input">
                        <input type="number" id="pastWeightValue" placeholder="重量" step="0.1">
                        <span>kg</span>
                        <input type="number" id="pastRepsValue" placeholder="回数" step="1">
                        <span>回</span>
                    </div>
                `;
                break;
            case 'custom_multi':
                container.innerHTML = `
                    <div class="custom-multi-input">
                        <input type="number" id="pastValue1" placeholder="値1" step="0.1">
                        <input type="text" id="pastUnit1" placeholder="単位1">
                        <input type="number" id="pastValue2" placeholder="値2" step="0.1">
                        <input type="text" id="pastUnit2" placeholder="単位2">
                    </div>
                `;
                break;
        }
    ...
    addPastCustomRecord() {
        ...
        const name = document.getElementById('pastCustomMenuName').value.trim();
        const type = document.getElementById('pastRecordType').value;
        ...
        switch (type) {
            case 'single':
                const value = document.getElementById('pastRecordValue')?.value;
                const unit = document.getElementById('pastRecordUnit')?.value.trim();
                if (value) {
                    values = { value: parseFloat(value), unit: unit || '' };
                    hasValidValue = true;
                }
                break;
            case 'distance_time':
                const distance = document.getElementById('pastDistanceValue')?.value;
                const time = document.getElementById('pastTimeValue')?.value;
                if (distance || time) {
                    values = {
                        distance: distance ? parseFloat(distance) : null,
                        distanceUnit: 'km',
                ...
                break;
            case 'weight_reps':
                const weight = document.getElementById('pastWeightValue')?.value;
                const reps = document.getElementById('pastRepsValue')?.value;
                if (weight || reps) {
                    values = {
                        weight: weight ? parseFloat(weight) : null,
                        weightUnit: 'kg',
                ...
                break;
            case 'custom_multi':
                const val1 = document.getElementById('pastValue1')?.value;
                const unit1 = document.getElementById('pastUnit1')?.value.trim();
                const val2 = document.getElementById('pastValue2')?.value;
                const unit2 = document.getElementById('pastUnit2')?.value.trim();
                if (val1 || val2) {
                    values = {
                        value1: val1 ? parseFloat(val1) : null,
                        unit1: unit1 || '',
        ...
        this.data.records[date].custom.push(record);
        this.saveData();
        this.renderCalendarView();
        this.showHistoryForDate(); // 履歴表示を更新

        // フォームクリア
        document.getElementById('pastCustomMenuName').value = '';
        this.updatePastCustomValueInputs();
    }
